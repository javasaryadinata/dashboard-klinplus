...
<h3 class="font-semibold text-3xl 2xl:text-4xl">
    Orders
</h3>
@endsection
...
<div x-data="{ isModalOpen: {{ $errors->any() ? 'true' : 'false' }} }"
    ...
    <div class="flex flex-row md:flex-row justify-between items-end md:items-center gap-4 mb-2">
        ...
        <div class="flex flex-row items-center gap-2 w-full md:max-w-sm z-20">
            <form method="GET" action="{{ route('orders.index') }}" autocomplete="off" class="relative w-full" onsubmit="event.preventDefault();">
                <div class="absolute inset-y-0 left-0 pl-3 flex text-gray-300 items-center pointer-events-none">
                    <x-lucide-search class="h-4 w-4 stroke-current stroke-[2.6]" />
                </div>
                <input type="text" id="searchInput" name="search" placeholder="Cari" value="{{ request('search') }}"
                    class="w-full h-9 bg-white border border-gray-200 pl-8 pr-8 py-2 rounded-xl text-gray-700 text-sm focus:border-1 focus:border-sky-400 outline-none transition-all shadow-sm">
                <button type="button" id="clearSearchBtn" class="absolute inset-y-0 right-0 mx-2 my-2 px-[2px] rounded-full flex items-center hover:bg-gray-100 text-gray-400 transition-colors {{ request('search') ? '' : 'hidden' }}">
                    <x-lucide-x class="h-4 w-4 stroke-current stroke-[1.6]" />
                </button>
            </form>
        </div>
        <button @click="isModalOpen = true; setTimeout(isiAlamatOtomatis, 100)" class="h-9 bg-sky-400 hover:bg-sky-500 text-white px-3 py-2 rounded-xl text-sm font-medium transition-colors shadow-sm flex items-center gap-1 whitespace-nowrap w-full md:w-auto justify-start cursor-pointer">
            <x-lucide-plus class="h-4 w-4 stroke-current stroke-[2.8]" />
            Tambah Order
        </button>
    </div>
            ...
            <table class="order-table w-full text-left">
                ...
                <tbody class="text-gray-200 divide-y">
                    ...
                    <tr class="hover:bg-gray-50/50">
                        ...
                        <td>
                            @php
                                $petugasUnik = $order->orderDetails->flatMap(function($detail) {
                                    return $detail->petugas;
                                })->pluck('nama_petugas')->unique();
                            @endphp

                            @if ($petugasUnik->isEmpty())
                                <div class="inline-flex items-center gap-1.5 px-2 py-[3px] rounded-lg text-xs text-red-600 bg-red-50 border border-red-100">
                                    <x-lucide-user-x class="w-3 h-3 stroke-[2]" /> Belum Ada
                                </div>
                            @else
                                <div class="flex items-center gap-1 flex-wrap max-w-[160px]">
                                    @foreach ($petugasUnik->take(2) as $nama)
                                        <span class="inline-flex items-center px-2 py-[3px] rounded-lg text-xs text-gray-700 bg-gray-100 border border-gray-200 truncate max-w-[100px]" title="{{ $nama }}">
                                            {{ explode(' ', trim($nama))[0] }}
                                        </span>
                                    @endforeach

                                    @if($petugasUnik->count() > 2)
                                        <span class="inline-flex items-center px-1.5 py-[3px] rounded-lg text-xs text-gray-500 bg-gray-100 border border-gray-200 cursor-help" title="{{ $petugasUnik->skip(2)->implode(', ') }}">
                                            +{{ $petugasUnik->count() - 2 }}
                                        </span>
                                    @endif
                                </div>
                        ...
                        <td class="relative text-center" x-data="{ openAction: false }">
                            ...
                            <div x-show="openAction" @click.away="openAction = false" x-transition.opacity.duration.200ms style="display: none;"
                                ...
                                <a href="{{ route('orders.detail', $order->id_order) }}" class="flex items-center gap-2 px-2 py-1.5 rounded-md text-xs text-gray-700 font-medium hover:bg-gray-100 transition-colors">
                                ...
                                @if($order->pelanggan && $order->pelanggan->telp_pelanggan)
                                    ...
                                    @endphp
                                    <a href="https://wa.me/{{ $waNumber }}" target="_blank" class="flex items-center px-2 py-1.5 gap-2 rounded-md text-xs text-gray-700 font-medium hover:bg-gray-100 transition-colors">
                                        <x-lucide-message-circle-more class="w-4 h-4 stroke-current stroke-[1.6]" />
                                        Hubungi Pelanggan
                                    </a>
                                @else
                                    <div class="flex items-center px-2 py-1.5 gap-2rounded-md text-xs text-gray-700 font-medium cursor-not-allowed" title="Nomor WA tidak tersedia">
                                        <x-lucide-message-circle-x class="w-4 h-4 stroke-current stroke-[1.6]" />
                                        Tidak ada WA
                                    </div>
                                @endif

                                <a href="{{ route('orders.invoicePdf', $order->id_order) }}" target="_blank" class="flex items-center px-2 py-1.5 gap-2 rounded-md text-xs text-gray-700 font-medium hover:bg-gray-100 transition-colors">
                                ...
                                <form action="{{ route('orders.approve', $order->id_order) }}" method="POST" class="w-full m-0">
                                    @csrf
                                    <button type="submit" class="w-full flex items-center px-2 py-1.5 gap-2 rounded-md text-xs text-gray-700 font-medium hover:bg-gray-100 transition-colors text-left">
                                        <x-lucide-clipboard-check class="w-4 h-4 stroke-[1.6]" />
                                        Terima Order
                                    </button>
                                </form>
                                ...
                                <form action="{{ route('orders.cancel', $order->id_order) }}" method="POST" class="w-full m-0" onsubmit="return confirm('Batalkan order ini?')">
                                    @csrf
                                    <button type="submit" class="w-full flex items-center px-2 py-1.5 gap-2 rounded-md text-xs text-gray-700 font-medium hover:bg-gray-100 transition-colors text-left">
                                        <x-lucide-clipboard-x class="w-4 h-4 stroke-[1.6]" />
                                        Tolak Order
                                    </button>
                                </form>
                                            ...
                                            <template x-for="p in filtered" :key="p.id">
                                                <li>
                                                    <a href="#" @click.prevent="selectPelanggan(p)" class="flex flex-col items-start px-3 py-2 hover:bg-slate-50 rounded-lg cursor-pointer">
                                                        <span class="font-medium text-gray-700 text-sm" x-text="p.nama"></span>
            ...
            <div x-show="isDeleteModalOpen" 
                ...
                <div x-show="isDeleteModalOpen" 
                    ...
                    <div class="flex justify-end gap-3 mt-6">
                        <button type="button" @click="isDeleteModalOpen = false" class="px-4 py-2 bg-gray-100 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-200 transition-colors">Batal</button>
